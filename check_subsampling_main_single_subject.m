function check_subsampling_main_single_subject()
...
%     load('simpleMeasurement16RegionsPar.mat');
    ...
    parfor i =1:num_repeat
        BRO_data = load(['results/simpleMeasurementAllRegions-1-3000',num2str(i),'-subjects.mat']');
        
        num_subjects = length(BRO_data.human6RandomResults);
        meanHuman6Results = zeros(size( BRO_data.human6Results{1} ) );
        meanHuman6RandomResults = [];
        for j = 1:num_subjects
            meanHuman6Results = meanHuman6Results + BRO_data.human6Results{j} / num_subjects;
            meanHuman6RandomResults = [meanHuman6RandomResults , BRO_data.human6RandomResults{j} ] ;
        end
        
        empirical_pvalue_huamn6_subset(:,i) = getEmpiricalPvalues( meanHuman6Results , meanHuman6RandomResults );
    ...
    saveFigure(gcf, sprintf('subsampling.png',i), 'png');
end
...
function plot_bars(empirical_pvalue_huamn6, empirical_pvalue_huamn6_subset, empirical_pvalue_kang)
    ...
    fprintf('subsampling mean %g (+- %g)\n',  mean(sum(empirical_pvalue_huamn6_subset < 0.01,1)), std(sum(empirical_pvalue_huamn6_subset < 0.01,1)) );
    hold on;
    bar(y);
    errorbar(y,e,'xr')
    hold off;
