...
%     gross_regions(gross_regions_inds) = true;
    ...
    numbeOfParentFromGrossRegions = sum(grossRegionChilds,1);
    assert(all(numbeOfParentFromGrossRegions <= 1) ,'cannot have nested regions, each small area should have at most one parent gross area');
